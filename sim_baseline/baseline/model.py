            ...
            for vid in video_indices:
                for rel_inst in dataset.get_relation_insts(vid, no_traj=True):
                    ...
                    for fstart, fend in segs:
                        ...
                        if self.extract_feature(vid, fstart, fend):
...
class SelectionLayer(Layer):
    ...
    def call(self, inputs, **kwargs):
        s = tf.keras.backend.gather(inputs[0], self.sel_inds[0], axis=1)
        p = tf.keras.backend.gather(inputs[1], self.sel_inds[1], axis=1)
        o = tf.keras.backend.gather(inputs[2], self.sel_inds[2], axis=1)
        return s * p * o
...
def train(dataset, param):
    ...
    training_model = build_model(param)

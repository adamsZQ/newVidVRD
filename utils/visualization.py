...
def get_objects_relations_list(data_type):
    objects_label_list = []
    objects_num_list = []
    relations_label_list = []
    relations_num_list = []
    objects_dict, relations_dict = get_objects_relations_dict(data_type)
    for each_object in sorted(objects_dict.items(), key=lambda item: item[1]):
        objects_label_list.append(each_object[0])
        objects_num_list.append(each_object[1])
    for each_relation in sorted(relations_dict.items(), key=lambda item: item[1]):
...
def statistic_visualization(label_list, num_list, title=None, bar_type=0):
    fontsize = 3
    if bar_type == 0:
        plt.bar(range(len(label_list)), num_list, color='rgb', tick_label=label_list)
        plt.yscale('log')
    ...
    plt.title(title)
    plt.savefig("{}.jpg".format(title), dpi=500)
    plt.show()

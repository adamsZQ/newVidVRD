...
def visualize(anno, video_path, out_path):
    ...
    assert anno['frame_count'] == len(video), '{} : anno {} video {}'.format(anno['video_id'], anno['frame_count'],
                                                                             len(video))
    ...
    for i in range(len(anno['trajectories'])):
        ...
        for x in sorted(f, key=lambda a: a['timestamp']):
            ...
            # font_size = map(lambda n: n * 1.5, font_size)
...
def extract_frames_2_gif(videos_path='vis_out/visualization'):
    os.system('mv ' + videos_path + '/*.mp4 ' + videos_path + '/videos/')
    ...
    for _, _, files in os.walk(videos_path + '/videos'):
        ...
        for each_file in files:
            ...
            os.system("convert -delay 20 -loop 0 " +
                      ...
                      + os.path.join(videos_path, "gifs", each_file[:-4] + ".gif"))

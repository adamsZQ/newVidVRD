...
class VTranse(object):
    ...
    def crop_pool_layer(self, bottom, rois, name):
        ...
        with tf.variable_scope(name) as scope:
            ...
            crops = tf.image.crop_and_resize(bottom, bboxes, tf.to_int32(batch_ids),
                                             [cfg.POOLING_SIZE * 2, cfg.POOLING_SIZE * 2],
                                             name="crops")
    ...
    def train_predicate(self, sess, roidb_use, RD_train):
        ...
        for batch_id in range(np.int32(batch_num)):
            ...
            feed_dict = {self.image: im, self.sbox: blob['sub_box'], self.obox: blob['obj_box'],
                         ...
                         self.keep_prob: 0.5}
            _, losses, predictions, layers = sess.run([RD_train, self.losses, self.predictions, self.layers],
                                                      feed_dict=feed_dict)
    ...
    def test_rela(self, sess, roidb_use):
        ...
        for batch_id in range(np.int32(batch_num)):
            ...
            predictions = sess.run(self.predictions, feed_dict=feed_dict)
            pred_rela[batch_id * self.N_each_batch:(batch_id + 1) * self.N_each_batch] = predictions['rela_pred'][:]
            pred_rela_score[batch_id * self.N_each_batch:(batch_id + 1) * self.N_each_batch] = predictions[
                                                                                                   'rela_max_prob'][:]
